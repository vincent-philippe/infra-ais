...
if [[ $EUID -ne 0 ]]; then
    echo "Ce script doit être lancé en sudo"
    exit 1
fi
...
cp "$DIR/manager.meta-data.yml" "/tmp/$domain_name.meta-data.yml"
...
cloud-localds "/tmp/$domain_name.iso" "$DIR/manager.user-data.yml" "/tmp/$domain_name.meta-data.yml"
